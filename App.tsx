...
import { 
  Lock, AlertTriangle, EyeOff, ShieldCheck, CreditCard, Eye, 
  Building, BookOpen, Check, Camera, Wifi, FileX, Package, X, 
  Building2, Ticket, AlertCircle, ArrowUpRight
} from 'lucide-react';
  ...
  <SlideContainer className="text-white flex items-center justify-center">
    ...
    >
      ...
      <motion.div variants={staggerContainer} className="grid grid-cols-3 gap-6 max-w-6xl mx-auto">
        {[
          { icon: Lock, color: '#ec3750', title: 'Legal Blackout', text: 'Minors under 18 cannot open business bank accounts. Robotics clubs, charities, and student events are locked out of the financial system.' },
          { icon: AlertTriangle, color: '#ff8c37', title: 'Cash Chaos', text: 'Forced to use teachers\' personal accounts, shoebox cash, or WhatsApp transfers. Non-compliant, uninsured, and impossible to audit.' },
          { icon: EyeOff, color: '#f1c40f', title: 'Zero Trust', text: 'Parents, schools, and sponsors refuse to fund initiatives they can\'t track. Students lose credibility before they even start.' }
        ].map((item, idx) => (
          <motion.div 
            key={idx}
            variants={fadeInUp}
    ...
    >
      <motion.div variants={fadeInUp} className="w-1/2">
        ...
        <div className="flex items-start gap-4 p-4 rounded-xl bg-[#ec3750]/5 border border-[#ec3750]/10">
          <AlertTriangle className="text-[#ec3750] w-6 h-6 mt-1 shrink-0" />
          <div>
  ...
  <SlideContainer className="text-white flex items-center justify-center">
    ...
    >
      <motion.div variants={fadeInUp} className="w-1/2">
        ...
        <p className="text-2xl text-gray-100 mb-8 font-light">Legal Infrastructure meets Gen Z Fintech.</p>
        <ul className="space-y-6">
          <li className="flex items-center gap-4">
            <div className="w-8 h-8 rounded-full bg-[#5bc0de]/20 flex items-center justify-center text-[#5bc0de]"><ShieldCheck className="w-5" /></div>
            <span className="text-lg">Trade License Coverage as Legal Umbrella</span>
          </li>
          <li className="flex items-center gap-4">
            <div className="w-8 h-8 rounded-full bg-[#5bc0de]/20 flex items-center justify-center text-[#5bc0de]"><CreditCard className="w-5" /></div>
            <span className="text-lg">Virtual debit cards issued in 10 seconds</span>
          </li>
          <li className="flex items-center gap-4">
            <div className="w-8 h-8 rounded-full bg-[#5bc0de]/20 flex items-center justify-center text-[#5bc0de]"><Eye className="w-5" /></div>
      ...
      >
        <div className="relative bg-slate-900/80 border border-white/10 rounded-3xl p-6 shadow-2xl transform rotate-3 hover:rotate-0 transition duration-500 hover:shadow-[0_0_30px_rgba(91,192,222,0.2)]">
          <div className="flex justify-between items-center mb-8">
            <div className="text-sm text-gray-200">Current Balance</div>
            <div className="w-8 h-8 rounded-full bg-gray-700"></div>
          </div>
          <div className="text-4xl font-semibold mb-2">AED <CountUp end={12450} />.00</div>
          <div className="text-[#33d6a6] text-sm mb-8">+ AED 2,400 (Bake Sale)</div>
          <div className="bg-gradient-to-br from-[#a633d6] to-[#7c25a0] rounded-xl p-6 shadow-lg mb-6 text-white relative overflow-hidden">
            <div className="flex justify-between mb-8">
              <span className="font-mono text-sm opacity-70">Horizon Virtual</span>
              <Wifi className="w-5 opacity-70" />
            </div>
            <div className="font-mono text-xl tracking-widest mb-4">•••• •••• •••• 4092</div>
            <div className="flex justify-between items-end">
              <div>
                <div className="text-xs opacity-50 uppercase">Holder</div>
                <div className="font-semibold">Robotics Club</div>
              </div>
  ...
  <SlideContainer className="text-white flex items-center justify-center">
    ...
    >
      <div className="w-3/4">
        ...
        <motion.h2 variants={fadeInUp} className="text-5xl font-semibold mb-5">Corporate Umbrella Model:<br /><span className="text-white">Banking Without Legal Complexity.</span></motion.h2>
        <motion.p variants={fadeInUp} className="text-xl text-gray-200 mb-10">
          We provide the Trade License coverage. Students get the banking infrastructure. Simple.
        </motion.p>
        <div className="space-y-8">
          {[
            { icon: Building, title: "Commercial License (We Handle It)", text: "Horizon provides the corporate structure. Students get compliant bank accounts without needing their own Trade License." },
            { icon: CreditCard, title: "Instant Debit Cards", text: "Issue virtual cards in 10 seconds. Buy event supplies, travel tickets, equipment—instantly." },
            { icon: BookOpen, title: "Public Transparency Ledger", text: "Every transaction is public. Parents, schools, sponsors see exactly where funds go. Trust by default." }
          ].map((item, idx) => (
      ...
      <motion.div variants={fadeInUp} className="w-1/4 relative">
        <div className="relative mx-auto border-gray-800 bg-gray-900 border-[8px] rounded-[2.5rem] h-[500px] w-[280px] shadow-2xl flex flex-col overflow-hidden transform hover:-translate-y-2 transition duration-500">
          <div className="h-[32px] bg-gray-800 absolute top-0 w-full left-0 z-20 rounded-b-[1rem]"></div>
          <div className="flex-1 bg-slate-950 p-4 pt-12 relative">
            <motion.div 
              initial={{ opacity: 0, y: -20, scale: 0.8 }}
              whileInView={{ opacity: 1, y: 0, scale: 1 }}
              transition={{ delay: 0.5, type: "spring" }}
              className="absolute top-16 left-4 right-4 bg-white/10 border border-white/20 p-3 rounded-lg flex gap-3 items-center"
            >
              <div className="w-8 h-8 bg-[#33d6a6] rounded-full flex items-center justify-center"><Check className="w-4 text-white" /></div>
              <div>
                <div className="text-xs text-gray-100">New Card Issued</div>
                <div className="text-xs font-semibold">Event Supplies • AED 500</div>
              </div>
            </motion.div>
            <div className="mt-20 space-y-4">
              <div className="bg-gradient-to-r from-[#a633d6] to-[#7c25a0] h-32 rounded-xl p-4 text-white shadow-lg">
                <div className="text-xs opacity-70 mb-4">Horizon Virtual</div>
                <div className="text-lg font-mono">•••• 4281</div>
              </div>
              <div className="text-gray-200 text-xs uppercase font-semibold tracking-wider">Recent</div>
              <div className="bg-white/5 p-3 rounded-lg flex justify-between items-center">
                <div className="text-sm">Amazon.ae</div>
                <div className="text-sm text-white">- AED 120.00</div>
              </div>
              <div className="bg-white/5 p-3 rounded-lg flex justify-between items-center">
                <div className="text-sm">Zoom Market</div>
                <div className="text-sm text-white">- AED 45.50</div>
              </div>
            </div>
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-16 h-16 bg-[#5bc0de] rounded-full flex items-center justify-center shadow-lg shadow-[#5bc0de]/50 cursor-pointer hover:scale-110 transition">
              <Camera className="w-8 h-8 text-semibold" />
            </div>
          </div>
        </div>
      </motion.div>
  ...
  <SlideContainer className="text-white flex items-center justify-center relative">
    ...
    >
      <motion.div variants={fadeInUp} className="mb-8">
        ...
        <h2 className="text-5xl font-semibold mb-4">
          "God Mode" Oversight.<br /><span className="text-gray-300">Every Transaction. Every Club. Real-Time.</span>
        </h2>
          ...
          <div className="col-span-5">
            ...
            <div className="w-full">
              ...
              <div className="grid grid-cols-4 text-sm py-3 border-t border-white/5 items-center bg-[#ec3750]/5">
                ...
                <div className="flex items-center gap-2 text-[#ec3750]"><AlertCircle className="w-4" /> Flagged</div>
              </div>
            </div>
          </div>
        </div>
    ...
    >
      ...
      <motion.div variants={staggerContainer} className="grid grid-cols-3 gap-6 w-full max-w-6xl">
        <motion.div variants={fadeInUp} className="relative bg-white/5 border border-white/10 p-8 rounded-3xl flex flex-col items-center text-center hover:bg-white/10 transition duration-300 group">
          ...
          <h3 className="text-2xl font-semibold mb-2">Interchange Fees</h3>
          ...
          <div className="text-3xl font-semibold text-white mb-2">0.5 - 1.5%</div>
          <p className="text-gray-200 text-sm">Earned on every transaction. Paid by merchants, not students. Scales automatically with spend volume.</p>
        </motion.div>
      ...
      <motion.div variants={fadeInUp} className="w-full max-w-5xl bg-gradient-to-r from-slate-900 to-slate-800 p-1 rounded-3xl border border-white/10 shadow-2xl">
        <div className="bg-[#0a0a0a]/80 rounded-[20px] px-10 py-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="text-3xl font-semibold text-white"><CountUp end={300} />k+ <span className="text-lg font-normal text-gray-200">AED</span></div>
              <div className="text-xs text-gray-300 uppercase font-semibold mt-1">Of Produce Processed</div>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={fadeInUp} className="relative bg-slate-900/50 border border-white/10 rounded-3xl overflow-hidden shadow-2xl w-full max-w-6xl">
        ...
        {[
          ...
          >
            ...
            <div>{row.cash ? <Check className="mx-auto text-green-500 w-5" /> : <X className="mx-auto text-[#ec3750] opacity-50 w-5" />}</div>
            <div>{row.bank ? <Check className="mx-auto text-green-500 w-5" /> : <X className="mx-auto text-[#ec3750] opacity-50 w-5" />}</div>
            <div>{row.b2c ? <Check className="mx-auto text-green-500 w-5" /> : <X className="mx-auto text-[#ec3750] opacity-50 w-5" />}</div>
            <div>
              {row.horizon ? (
                <div className="w-8 h-8 bg-[#5bc0de] rounded-full flex items-center justify-center mx-auto shadow-lg shadow-[#5bc0de]/50 scale-110">
                  <Check className="text-semibold w-5 h-5" />
                </div>
              ) : <X className="mx-auto text-[#ec3750] opacity-50 w-5" />}
            </div>
          </motion.div>
        ))}
      </motion.div>
